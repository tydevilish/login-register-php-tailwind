...
class Register extends connection
{
    ...
    {
        if (empty($username)) {
            $this->redirectWithError('Please enter your username');
        }
        return true;
    }
    ...
    public function emailCheck($email)
    {
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
            $this->redirectWithError('Invalid email format');
        }
        return true;
    }
    ...
    {
        if (!isset($checkbox)) {
            $this->redirectWithError('Read the Terms and Conditions and fill out the checkbox.');
        }
        return true;
    }

    public function existsUsers($username, $email)
    {
        ...
        if ($countUser > 0) {
            $this->redirectWithError('Username already exists');
        } elseif ($countEmail > 0) {
    ...
    public function insertUsers($username, $password, $email)
    {
...
if (isset($_POST['submit'])) {
    ...
    $register->usernameCheck($username);
    $register->passwordCheck($password, $confirm_password);
    $register->emailCheck($email);
    $register->checkBox($checkbox);
    $register->existsUsers($username, $email);
    $register->insertUsers($username, $password, $email);
}
        ...
        <div class=" flex flex-wrap items-center  mx-auto p-4 fixed">
            ...
            <div class="hidden w-full" id="navbar-hamburger">
                <ul class="flex flex-col font-medium rounded-lg shadow-2xl bg-gray-50 dark:bg-gray-900 dark:border-gray-700">
                    ...
                    <li>
                        <a href="signin.php" class="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">Sign in</a>
                    </li>
                </ul>
            </div>
        </div>
                ...
                <form class="space-y-4 md:space-y-6" method="post">
                    ...
                    <div class="flex items-start">
                        <div class="flex items-center h-5">
                            <input id="terms" aria-describedby="terms" name="checkbox" type="checkbox" class="w-4 h-4 cursor-pointer appearance-none border border-gray-300 rounded bg-gray-50 checked:bg-blue-600 checked:border-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-300 focus:ring-offset-2 dark:bg-gray-700 dark:border-gray-600 dark:checked:bg-blue-600 dark:checked:border-blue-600 dark:focus:ring-blue-800 dark:focus:ring-offset-gray-800" required>
                        </div>
                        <div class="ml-3 text-sm">
                            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">
                                I accept the <a class="font-medium text-blue-500 cursor-pointer" id="terms-link">Terms and Conditions</a>
                            </p>
                        </div>
                    </div>

                    <button type="submit" name="submit" class="w-full text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 dark:text-white">Create an account</button>
                    <p class="text-sm font-medium text-gray-500 dark:text-gray-400">
                        Already have an account? <a href="signin.php" class="font-medium text-blue-600 hover:underline dark:text-blue-500">Login here</a>
                    </p>
                    <?php
                    ?>
                </form>
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-4 md:p-5 space-y-4 overflow-y-auto max-h-96">
                    <p class="text-base leading-relaxed text-gray-500 dark:text-gray-400">
                        Personal information will be protected under our privacy policy and will not be disclosed without consent.
                    </p>
                </div>
                <div class="flex items-center p-4 md:p-5 border-t border-gray-200 rounded-b dark:border-gray-600">
                    <button id="accept-terms" type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">I accept</button>
                    <button id="decline-terms" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">Decline</button>
                </div>
            </div>
        </div>
